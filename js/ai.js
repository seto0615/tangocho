    ...
    items: {
      ...
      items: {
        ...
        properties: {
          ...
          note: { type: 'string', description: '品詞・例文・補足。なければ空文字' },
...
- 英語と日本語訳はレイアウト上ばらばらに配置されていることがあります。位置関係・番号・矢印・色・意味から正しい相手を判断して対応づけてください。
- en は素材の表記どおりに。文頭でない一般語は小文字のまま、固有名詞や文は大文字を保ちます。行頭の番号・記号・チェックボックスは除きます。
- ja は素材に書かれている訳をそのまま使います。訳が見当たらない英語には、学習者向けの簡潔で自然な訳を補い、generated を true にします。
- 品詞・発音記号・例文・言い換えなど、その語に紐づく補足が素材にあれば note に短くまとめます。なければ空文字にします。
- アプリのメニュー、時刻、バッテリー表示、広告、ボタンのラベルなど、学習内容でないUI文字列は無視します。
